<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Editar Usuário — {{ $user->name }}</h2>
    </x-slot>
                ...
                <form method="POST" action="{{ route('users.update', $user->id) }}">
                    ...
                    <div class="mb-6">
                        ...
                        <select name="role" id="role" onchange="togglePermissions()"
                                class="w-full border border-gray-300 rounded px-3 py-2">
                            <option value="admin" {{ old('role', $userRole) === 'admin' ? 'selected' : '' }}>Administrador</option>
                            <option value="collaborator" {{ old('role', $userRole) === 'collaborator' ? 'selected' : '' }}>Colaborador</option>
                        </select>
                    </div>
                    ...
                    <div class="flex gap-3">
                        ...
                        <a href="{{ route('users.index') }}" class="bg-gray-100 text-gray-700 px-6 py-2 rounded hover:bg-gray-200">
                            Cancelar
                        </a>
                    </div>
                </form>
